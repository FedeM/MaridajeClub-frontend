        ...
        return(
            <div className={styles.modal_background}>
                <div className={`${styles.modal_container} animate__animated animate__lightSpeedInRight animate__faster`}>
                    <h4 onClick={()=> enterEvent()}>{modalActivate.event.name}</h4>
                    ...
                    {!modalActivate.isNext && (<button onClick={()=> enterEvent()}>Ingresar al evento</button>)}
            ...
            <div className={styles.live_wrapper} id='live'>
                {
                    homeEvent.inLiveEvent ?(
                        <LiveEvent event={homeEvent.inLiveEvent} helpOpacity={helpOpacity} setModalActivate={setModalActivate}/>
                    ): homeEvent.nextEvent ?(
                        <NextEvent event={homeEvent.nextEvent} helpOpacity={helpOpacity} setModalActivate={setModalActivate}/>
                    ): homeEvent.lastEvent ?(
                        <LiveEvent event={homeEvent.lastEvent} helpOpacity={helpOpacity} last setModalActivate={setModalActivate}/>
                    ):('')
                }
            </div>
        </div>
